...
function renderComments(response) {
    ...
    list.innerHTML = response.comments.map(c => `
        <div class="comment-card" id="row-${CSS.escape(c.timestamp)}">
            <div class="comment-info">
                <strong>${c.nama}</strong>
                <small>${c.postId} &middot; ${c.timestamp}</small>
                <p>${c.komentar}</p>
            </div>
            <div class="btn-group">
...
async function updateStatus(timestamp, status) {
    const safeId = CSS.escape(timestamp);
    const card   = document.getElementById(`row-${safeId}`);
    if (!card) return;

    // Tandai sedang diproses — jangan hapus dulu sebelum server konfirmasi
    ...
    // Hapus dari DOM hanya setelah fetch selesai (tidak ada error jaringan)
    card.remove();
}
